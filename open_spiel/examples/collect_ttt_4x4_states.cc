...
std::tuple<std::vector<double>, std::vector<std::string>>
PlayGame(const open_spiel::Game &game,
  ...
  return {state->Returns(), state_history};
}
...
int main(int argc, char **argv) {
  ...
  // Ensure the game and arguments are compatible.
  open_spiel::GameType game_type = game->GetType();
  if (game->NumPlayers() != 2)
    open_spiel::SpielFatalError("Can only handle 2-player games.");
  ...
  std::vector<std::unique_ptr<open_spiel::Bot>> bots;
  bots.push_back(
      InitBot(absl::GetFlag(FLAGS_player1), *game, 0, evaluator));
  bots.push_back(
      InitBot(absl::GetFlag(FLAGS_player2), *game, 1, evaluator));
  ...
  if (outFile.is_open()) {
    for (int game_num = 0; game_num < num_games; ++game_num) {
      // play game
      auto [returns, history] = PlayGame(*game, bots, rng, absl::GetFlag(FLAGS_move_log_cutoff));
