...
ArgsLibAddArg num_threads int -1 'Number of threads to use when paralellizing build  tests. (Defaults to 4number of CPUs)'
ArgsLibParse $@
...
if [[ $VIRTUAL_ENV !=  ]]
then
  echo -e e[1me[93mVirtualenv already detected. We do not create a new one.e[0m
  ArgsLibSet virtualenv false
fi

echo -e e[33mRunning ${0} from $PWDe[0m
PYBIN=${PYBIN-python3}
PYBIN=`which ${PYBIN}`
if [ ! -x $PYBIN ]
then
  echo -e e[1me[93m$PYBIN not found! Skip build and test.e[0m
  continue
fi
...
if [[ ${OPEN_SPIEL_BUILD_WITH_JULIA-OFF} == ON ]]; then
  ...
  if [[ ! -x `which julia` ]]  [[ $(julia -e 'println(VERSION = v1.6.0-rc1)') == false ]]
  then
    echo -e e[33mWarning julia not in your PATH or it's too old. Trying $HOME.localbine[0m
    PATH=${HOME}.localbin${PATH}
    [[ -x `which julia` ]] && [[ $(julia -e 'println(VERSION = v1.6.0-rc1)') == true ]]  die could not find julia command. Please add it to PATH and rerun.
  fi
...
function execute_export_graph {
  ...
  python ..open_spielcontribpythonexport_graph.py
}
...
if [[ $ARG_build_with_pip == true ]]; then
  ...
  export PYTHONPATH=$PYTHONPATH$pwdpython  # For pyspiel bindings
