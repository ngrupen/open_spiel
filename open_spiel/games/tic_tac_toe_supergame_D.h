...
#ifndef OPEN_SPIEL_GAMES_TIC_TAC_TOE_SUPERGAME_D_H_
#define OPEN_SPIEL_GAMES_TIC_TAC_TOE_SUPERGAME_D_H_
...
class TicTacToeSuperGameDState : public State {
 public:
  TicTacToeSuperGameDState(std::shared_ptr<const Game> game);

  TicTacToeSuperGameDState(const TicTacToeSuperGameDState&) = default;
  TicTacToeSuperGameDState& operator=(const TicTacToeSuperGameDState&) = default;
...
// Game object.
class TicTacToeSuperGameDGame : public Game {
 public:
  explicit TicTacToeSuperGameDGame(const GameParameters& params);
  int NumDistinctActions() const override { return kNumCells*3+Summation(kNumCells-1)*2; }
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new TicTacToeSuperGameDState(shared_from_this()));
  }
...
#endif  // OPEN_SPIEL_GAMES_TIC_TAC_TOE_SUPERGAME_D_H_
