...
#include "open_spiel/games/tic_tac_toe_supergame_F.h"
...
const GameType kGameType{
    /*short_name=*/"tic_tac_toe_supergame_F",
    /*long_name=*/"Tic Tac Toe (SuperGame F)",
...
std::shared_ptr<const Game> Factory(const GameParameters& params) {
  return std::shared_ptr<const Game>(new TicTacToeSuperGameFGame(params));
}
...
void TicTacToeSuperGameFState::DoApplyAction(Action move) {
...
std::vector<Action> TicTacToeSuperGameFState::LegalActions() const {
  if (IsTerminal()) return {};

  // can move in any cell, with any marker, erase, or add two markers of the same type
  std::vector<Action> moves;
  for (int cell = 0; cell < (kNumCells*3+Summation(kNumCells-1)*9); ++cell) {
    moves.push_back(cell);
  }
  return moves;
}

std::vector<Action> TicTacToeSuperGameFState::OriginalLegalActions(Player player) {
  if (IsTerminal()) return {};
...
int TicTacToeSuperGameFState::CheckValidMove(Player player, Action action) {
...
CellState ActionToMarker(Action action_id) {
  ...
  // o and erase
  if (action_id >= kNumCells*3+Summation(kNumCells-1)*8) {
    return CellState::kNought;
  }
  // x and erase
  else if (action_id >= kNumCells*3+Summation(kNumCells-1)*7) {
    return CellState::kCross;
  }
  // erase and o
  else if (action_id >= kNumCells*3+Summation(kNumCells-1)*6) {
    return CellState::kEmpty;
  }
  // erase and x
  else if (action_id >= kNumCells*3+Summation(kNumCells-1)*5) {
    return CellState::kEmpty;
  }
  // two erases
  else if (action_id >= kNumCells*3+Summation(kNumCells-1)*4) {
    return CellState::kEmpty;
  }
  // o and x
  else if (action_id >= kNumCells*3+Summation(kNumCells-1)*3) {
    return CellState::kNought;
  }
  // x and o
...
CellState ActionToMarker2(Action action_id) {
  ...
  // no second marker
    ...
    SpielFatalError(absl::StrCat("No second marker, action id: ", action_id));
    return CellState::kEmpty;
  }
}
...
std::string TicTacToeSuperGameFState::ActionToString(Player player,
...
bool TicTacToeSuperGameFState::HasLine(Player player) const {
...
bool TicTacToeSuperGameFState::IsFull() const { return num_moves_ == kNumCells; }

TicTacToeSuperGameFState::TicTacToeSuperGameFState(std::shared_ptr<const Game> game) : State(game) {
  std::fill(begin(board_), end(board_), CellState::kEmpty);
}

std::string TicTacToeSuperGameFState::ToString() const {
  std::string str;
...
bool TicTacToeSuperGameFState::IsTerminal() const {
  return outcome_ != kInvalidPlayer || IsFull();
}

std::vector<double> TicTacToeSuperGameFState::Returns() const {
...
std::string TicTacToeSuperGameFState::InformationStateString(Player player) const {
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
  return HistoryString();
}

std::string TicTacToeSuperGameFState::ObservationString(Player player) const {
  SPIEL_CHECK_GE(player, 0);
  SPIEL_CHECK_LT(player, num_players_);
  return ToString();
}

void TicTacToeSuperGameFState::ObservationTensor(Player player,
...
void TicTacToeSuperGameFState::UndoAction(Player player, Action move) {
...
std::unique_ptr<State> TicTacToeSuperGameFState::Clone() const {
  return std::unique_ptr<State>(new TicTacToeSuperGameFState(*this));
}

TicTacToeSuperGameFGame::TicTacToeSuperGameFGame(const GameParameters& params)
